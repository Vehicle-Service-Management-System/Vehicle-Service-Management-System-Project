...
router.get('/:id', (req, res) => {
  try {
    ...
    // Get services for each vehicle
    const vehicleIds = vehicles.map(v => v.id);
    let services = [];
    if (vehicleIds.length > 0) {
      services = db.prepare(`
        SELECT s.*, v.make, v.model, v.reg_number, su.name as added_by_name
        FROM services s
        JOIN vehicles v ON s.vehicle_id = v.id
        LEFT JOIN users su ON s.added_by = su.id
        WHERE s.vehicle_id IN (${vehicleIds.map(() => '?').join(',')}) AND s.centre_id = ?
        ORDER BY s.service_date DESC
      `).all(...vehicleIds, req.user.centreId);
    }

    res.json({ customer, vehicles, services });
  } catch (err) {
